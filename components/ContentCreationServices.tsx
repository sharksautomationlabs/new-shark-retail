...
import { ArrowRight, Layout, Image, FileText, Zap } from "lucide-react";
...
const services = [
  { title: "Content Strategy That Works", desc: "Complete content strategy designed for real results. Advanced planning, brand voice development, content performance analysis, and ROI tracking.", icon: Layout },
  { title: "Visual Content Excellence", desc: "Professional visual content that builds brand recognition and drives conversion. High-converting imagery, video production, and e-commerce assets.", icon: Image },
  { title: "Copywriting Solutions", desc: "Conversion-optimized copywriting across all channels. SEO-enhanced content, brand voice, email marketing, and product descriptions.", icon: FileText },
  { title: "Content Production Automation", desc: "Intelligent workflows that eliminate bottlenecks and accelerate content operations. Automated workflows, templates, and quality control.", icon: Zap },
];
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            Content Creation <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">Solutions</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-gray-400 text-lg max-w-3xl mx-auto font-medium">
            Our comprehensive content creation services enhance your brand presence, drive engagement, and accelerate content marketing success.
          </motion.p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-12">
          <a href="/contact" className="inline-flex items-center gap-3 rounded-full bg-teal-400 text-black font-extrabold px-8 py-4 text-sm uppercase tracking-wider hover:bg-teal-300 transition-colors duration-300 shadow-[0_0_40px_rgba(20,184,166,0.25)]">
            Schedule Your Capital Intro Call <ArrowRight className="w-5 h-5" />
          </a>
        </motion.div>
      </div>
    </section>
