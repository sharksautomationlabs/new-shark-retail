...
import { ArrowRight, Search, BarChart3, Package, TrendingUp } from "lucide-react";
...
const cards = [
  { label: "PH—01", title: "Product Discovery", desc: "Advanced research and trend analysis for winning product identification. We uncover opportunities before they become saturated.", icon: Search },
  { label: "PH—02", title: "Market Analysis", desc: "Comprehensive market intelligence that drives strategic advantage. Competitor mapping and saturation assessment.", icon: BarChart3 },
  { label: "PH—03", title: "Supplier Sourcing", desc: "Strategic supplier partnerships that maximize profit margins. Verification, negotiation, and quality assurance.", icon: Package },
  { label: "PH—04", title: "E-commerce Growth", desc: "Accelerating growth through product portfolio optimization. Cross-platform scaling and market validation.", icon: TrendingUp },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-20 items-center">
          <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="lg:col-span-5 space-y-8">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 rounded-full border border-teal-500/20 bg-teal-500/5 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.25em] text-teal-400 backdrop-blur-md">
              <span className="w-1.5 h-1.5 rounded-full bg-teal-400 shadow-[0_0_8px_teal]" /> Product Hunting
            </motion.div>
            <motion.h2 variants={fadeUp} className="text-3xl sm:text-[2.15rem] md:text-[2.6rem] lg:text-[3rem] font-bold text-white tracking-tight leading-[1.15]">
              Inventory decisions backed by <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">merchandising intelligence</span>
            </motion.h2>
            <motion.p variants={fadeUp} className="text-slate-400 text-lg md:text-xl leading-relaxed font-light">
              Retail Automation delivers product discovery, market analysis, supplier sourcing, and e-commerce growth optimization. We help brands scale smarter and sell faster.
            </motion.p>
            <motion.p variants={fadeUp} className="text-slate-400 text-base md:text-lg leading-relaxed font-light">
              From trend identification and competitor intelligence to supplier verification and portfolio strategy—we tailor research for your goals. Built for scale, designed for results.
            </motion.p>
