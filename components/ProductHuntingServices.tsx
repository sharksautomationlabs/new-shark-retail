...
import { ArrowRight, TrendingUp, Users, Package, Target } from "lucide-react";
...
const services = [
  { title: "Trend Analysis & Discovery", desc: "Identify emerging trends and winning products before they become saturated. Social media monitoring, Google Trends, and market saturation assessment.", icon: TrendingUp },
  { title: "Competitor Intelligence", desc: "Comprehensive analysis of competitor strategies and market positioning. Competitor product mapping, pricing analysis, and gap identification.", icon: Users },
  { title: "Supplier Research & Sourcing", desc: "Find reliable suppliers and negotiate optimal terms. Supplier verification, quality assessment, pricing negotiation, and logistics analysis.", icon: Package },
  { title: "Market Validation Testing", desc: "Validate product-market fit before significant investment. Pre-launch testing, customer feedback, demand forecasting, and risk assessment.", icon: Target },
];
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            Product Hunting <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">Solutions</span>
          </motion.h2>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-12">
          <a href="/contact" className="inline-flex items-center gap-3 rounded-full bg-teal-400 text-black font-extrabold px-8 py-4 text-sm uppercase tracking-wider hover:bg-teal-300 transition-colors duration-300 shadow-[0_0_40px_rgba(20,184,166,0.25)]">
            Schedule Your Discovery Call <ArrowRight className="w-5 h-5" />
          </a>
        </motion.div>
      </div>
    </section>
