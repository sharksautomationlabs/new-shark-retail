...
import { ArrowRight, Megaphone, Search, Share2, Mail, BarChart3 } from "lucide-react";
...
const services = [
  { title: "PPC & Paid Advertising", desc: "Google Ads, Meta, Amazon PPC, TikTok Ads—precision campaigns that maximize ROAS and scale profitable spend.", icon: Megaphone },
  { title: "SEO & Organic Traffic", desc: "Keyword research, on-page optimization, and content strategy to rank higher and drive sustainable organic traffic.", icon: Search },
  { title: "Social Media Marketing", desc: "Brand awareness and engagement across Instagram, Facebook, TikTok, and Pinterest for e-commerce growth.", icon: Share2 },
  { title: "Content & Email Marketing", desc: "Strategic content creation, email campaigns, and retention programs to nurture leads and drive repeat sales.", icon: Mail },
  { title: "Analytics & Reporting", desc: "Performance dashboards, conversion tracking, and data-driven insights to optimize every marketing channel.", icon: BarChart3 },
];
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.03] border border-white/[0.05] backdrop-blur-md mb-6">
            ...
            <span className="text-xs font-bold text-gray-300 uppercase tracking-[0.2em]">Services</span>
          </motion.div>
          <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            Digital Marketing <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">Services</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-gray-400 text-lg max-w-3xl mx-auto font-medium">
            From basic marketing support to advanced campaigns—we deliver strategies that scale your e-commerce brand.
          </motion.p>
        </motion.div>

        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
