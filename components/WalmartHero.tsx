...
const fadeUpVariant = { hidden: { opacity: 0, y: 30, filter: "blur(8px)" }, visible: { opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: 1, ease: [0.2, 0.65, 0.3, 0.9] as const } } };
  ...
  return (
    <section className="relative bg-[#030303] min-h-[85vh] lg:min-h-[90vh] flex items-center justify-center py-20 px-4 sm:px-6 lg:px-12 overflow-hidden font-sans selection:bg-teal-500/30 selection:text-teal-100">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex flex-col items-center lg:items-start text-center lg:text-left max-w-3xl">
          <motion.div variants={fadeUpVariant} className="mb-6 sm:mb-8">
            <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white/[0.03] border border-white/[0.08] backdrop-blur-md shadow-2xl">
              ...
              <span className="text-xs sm:text-sm font-semibold text-gray-300 uppercase tracking-[0.2em]">Walmart</span>
            </div>
          </motion.div>
          <motion.h1 variants={fadeUpVariant} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-white mb-6 leading-[1.1] tracking-tight">
            We Always Give The Best <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-teal-200 drop-shadow-[0_0_30px_rgba(20,184,166,0.3)]">Walmart Marketplace Service</span> To You
          </motion.h1>
          <motion.p variants={fadeUpVariant} className="text-base sm:text-lg md:text-xl text-gray-400 leading-relaxed font-medium mb-10">
            Boost your digital presence on Walmart with Shark Retail. We help you get approved for Walmart Marketplace, set up WFS (Walmart Fulfillment Services), and scale your sales on the world&apos;s largest retail platform.
          </motion.p>
          <motion.div variants={fadeUpVariant} className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <a href="/contact" className="relative group overflow-hidden rounded-full shadow-[0_0_40px_rgba(20,184,166,0.2)] hover:shadow-[0_0_60px_rgba(20,184,166,0.4)] transition-shadow duration-500">
              ...
              <div className="relative px-8 py-4 flex items-center justify-center gap-3">
                ...
                <div className="bg-black/10 rounded-full p-1 group-hover:bg-black group-hover:text-teal-400 transition-colors duration-300"><ArrowRight className="w-5 h-5 text-black group-hover:text-teal-400 transition-colors" /></div>
              </div>
            </a>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, delay: 0.4, ease: [0.2, 0.65, 0.3, 0.9] }} className="relative shrink-0">
          <div className="relative w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80 lg:w-96 lg:h-96">
            <div className="absolute inset-0 bg-teal-400/20 rounded-full blur-3xl scale-110" />
            <div className="relative w-full h-full flex items-center justify-center">
              <Image src="/images/companies/processed/walmart.png" alt="Walmart" width={384} height={384} className="w-full h-full object-contain opacity-95 filter brightness-0 invert" />
            </div>
          </div>
        </motion.div>
      </div>
