...
import { ChevronDown, Phone, TrendingUp, Mail, MapPin } from "lucide-react";
...
const Checkbox: React.FC<CheckboxProps> = ({ label, id, checked, onChange }) => (
  <div className="flex items-center">
    ...
    />
    <label htmlFor={id} className="ml-2 text-sm text-gray-400">
      {label}
    </label>
  </div>
);

// --- Amazon PPC CTA Component ---
const AmazonPPCCTA: React.FC = () => {
  ...
  const servicesOptions = [
    ...
    "Campaign Structure & Audits",
    "Performance Reporting & Analytics",
    "Competitor Analysis",
    "Other Amazon PPC Services",
  ];
  ...
  const formatPhoneNumber = (value: string, countryCode: string) => {
    ...
    if (countryCode === "+1") {
      ...
      if (phoneDigits.length <= 6)
        return `${countryCode} (${phoneDigits.slice(
          0,
          3
        )}) ${phoneDigits.slice(3)}`;
      return `${countryCode} (${phoneDigits.slice(0, 3)}) ${phoneDigits.slice(
        3,
        6
      ...
      return `${countryCode} ${phoneDigits.slice(0, 4)} ${phoneDigits.slice(
        4,
        10
      )}`;
      ...
      return `${countryCode} ${phoneDigits.slice(0, 5)} ${phoneDigits.slice(
        5,
        10
      )}`;
    } else if (
      ...
      return `${countryCode} ${phoneDigits.slice(
        0,
        1
      )} ${phoneDigits.slice(1, 4)} ${phoneDigits.slice(4, 8)}`;
    } else {
      if (phoneDigits.length === 0) return countryCode + " ";
      if (phoneDigits.length <= 4) return `${countryCode} ${phoneDigits}`;
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          jobInquiry:
            formData.jobInquiry !== "Please Select" ? formData.jobInquiry : "",
        }),
      });
    ...
    <section className="relative bg-[#020202] py-24 sm:py-32 px-4 sm:px-8 lg:px-12 overflow-hidden font-sans selection:bg-teal-500/30 selection:text-white">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        >
          <motion.div
            variants={fadeUp}
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-teal-500/30 bg-teal-500/10 backdrop-blur-md shadow-[0_0_20px_rgba(20,184,166,0.15)] mb-6"
          >
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-teal-500" />
            </span>
            <span className="text-[10px] font-bold text-teal-400 uppercase tracking-[0.3em]">
              Amazon PPC Management
            </span>
          </motion.div>
          <motion.h2
            variants={fadeUp}
            className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight mb-4 leading-[1.1]"
          >
          ...
          <motion.p
            variants={fadeUp}
            className="text-lg text-gray-400 max-w-2xl mx-auto font-medium mb-12"
          >
            Ready to cut ACOS and boost ROAS? Share a few details and our team
            will design a premium Amazon PPC strategy tailored to your store.
          </motion.p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 items-start">
          ...
          >
            ...
            <div className="relative bg-[#0a0a0c]/90 backdrop-blur-2xl border border-white/10 p-6 sm:p-8 rounded-[2rem]">
              <h3 className="text-2xl font-bold text-white mb-2">
                Premium Amazon PPC Strategy For Your Store
              </h3>
              <p className="text-sm text-gray-400 mb-6 sm:mb-8">
                Tell us about your Amazon advertising and we&apos;ll design a
                data-driven PPC strategy to lower ACOS and boost ROAS.
              </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                ...
                <FormField
                  label="Tell us about your Amazon advertising goals"
                  as="textarea"
                  name="projectDetails"
          ...
          >
            {[
              {
                icon: TrendingUp,
                title: "Scale Your Amazon Advertising",
                desc: "Data-driven PPC optimization for Sponsored Products, Brands, and Display—lower ACOS, higher ROAS.",
              },
              {
                icon: Mail,
                title: "Email",
              ...
              <div
                key={i}
                className="relative group p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-teal-500/30 hover:bg-white/[0.04] transition-all duration-500"
              >
