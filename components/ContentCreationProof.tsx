...
const fadeUp = { hidden: { opacity: 0, y: 40 }, visible: { opacity: 1, y: 0, transition: { type: "spring" as const, stiffness: 80, damping: 20 } } };
const staggerContainer = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.1 } } };
const targetCounts = { projects: 3500, engagement: 85, brands: 450, satisfaction: 97 };
const statLabels: Record<string, string> = { projects: "Content Projects", engagement: "Engagement Rate", brands: "Brands Served", satisfaction: "Client Satisfaction" };
const statSuffixes: Record<string, string> = { projects: "+", engagement: "%", brands: "+", satisfaction: "%" };

const ContentCreationProof: React.FC = () => {
  const [counts, setCounts] = useState({ projects: 0, engagement: 0, brands: 0, satisfaction: 0 });
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            Building The Content of <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">Tomorrow, Today</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-gray-400 text-lg max-w-3xl mx-auto font-medium">
            We help brands create compelling content with measurable results.
          </motion.p>
        </motion.div>
