...
import { ArrowRight, PenLine, Image, FileText, Zap } from "lucide-react";
...
const cards = [
  { label: "CC—01", title: "Creative Strategy", desc: "Data-driven content strategy that scales brand presence. Advanced planning, brand voice development, and performance analytics.", icon: PenLine },
  { label: "CC—02", title: "Visual Content", desc: "Professional visual content that drives conversion. High-converting imagery, video production, and e-commerce product assets.", icon: Image },
  { label: "CC—03", title: "Copywriting", desc: "Conversion-optimized copywriting that drives results. SEO-enhanced content, brand voice, and product descriptions.", icon: FileText },
  { label: "CC—04", title: "Production Automation", desc: "Intelligent workflows that accelerate content operations. Automated workflows, template systems, and quality control.", icon: Zap },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-20 items-center">
          <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="lg:col-span-5 space-y-8">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 rounded-full border border-teal-500/20 bg-teal-500/5 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.25em] text-teal-400 backdrop-blur-md">
              <span className="w-1.5 h-1.5 rounded-full bg-teal-400 shadow-[0_0_8px_teal]" /> Content Creation
            </motion.div>
            <motion.h2 variants={fadeUp} className="text-3xl sm:text-[2.15rem] md:text-[2.6rem] lg:text-[3rem] font-bold text-white tracking-tight leading-[1.15]">
              Sky-Rocket Your Brand with <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-teal-400 to-emerald-300">Elite Content Creation</span>
            </motion.h2>
            <motion.p variants={fadeUp} className="text-slate-400 text-lg md:text-xl leading-relaxed font-light">
              Shark Retail delivers creative strategy, visual content, copywriting, and production automation. We help brands scale smarter and sell faster.
            </motion.p>
            <motion.p variants={fadeUp} className="text-slate-400 text-base md:text-lg leading-relaxed font-light">
              From content planning and brand voice to visual assets and workflow automation—we tailor creative solutions for your goals. Built for scale, designed for results.
            </motion.p>
