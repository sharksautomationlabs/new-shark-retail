...
const ProductHuntingEdge: React.FC = () => {
  ...
  const advantages = [
    { title: "Data-Driven Product Discovery", description: "Uncover winning products through advanced market analysis and trend identification." },
    { title: "Competitive Intelligence", description: "Stay ahead with real-time competitor analysis and market gap identification." },
    { title: "Profit Optimization", description: "Maximize margins with strategic pricing and supplier negotiation expertise." },
    { title: "Scalable Growth", description: "Build sustainable product portfolios that scale across multiple marketplaces." },
  ];

  const challenges = [
    { title: "Trend Identification", description: "Without proper research, you're chasing outdated trends and wasting valuable time and resources." },
    { title: "Competitor Analysis", description: "Manual competitor research is time-consuming and often misses critical market insights." },
    { title: "Supplier Relations", description: "Finding reliable suppliers and negotiating favorable terms without experience is challenging." },
  ];
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        >
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.03] border border-white/[0.05] backdrop-blur-md mb-6">
            ...
            <span className="text-xs font-bold text-gray-300 uppercase tracking-[0.2em]">The Edge</span>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16">
          <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="space-y-6">
            ...
            {advantages.map((item) => (
              <motion.div key={item.title} variants={fadeUp} className="rounded-2xl border border-white/[0.06] bg-[#0a0a0c]/80 backdrop-blur-xl p-6 shadow-2xl shadow-black/40 hover:border-teal-500/20 transition-all duration-500">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? "visible" : "hidden"} className="space-y-6">
            <div className="text-center mb-8">
              <h3 className="text-xl font-bold text-red-400/90 mb-2">DIY (Going Solo)</h3>
              <div className="w-16 h-1 bg-red-500/60 mx-auto rounded-full" />
            </div>
            {challenges.map((item) => (
              <motion.div key={item.title} variants={fadeUp} className="rounded-2xl border border-white/[0.04] bg-[#0a0a0c]/60 backdrop-blur-xl p-6 border-red-400/10">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-white/80 mb-2">{item.title}</h4>
